...
void Iint(unsigned int instr, DecodedInstr* d, RegVals* rVals){
    ...
    if ( d->regs.i.addr_or_immed>32767){
        //signextended goes from 32767 to -32767
  ...
  *changedMem = -1;
   ...
   if (d->op==43){
       ...
       if( 0x00401000>val|| val>0x00403fff ||val%4!=0){
         ...
         //  unsigned int addy;
          // addy = Fetch(val);
      //mips.memory[addy]=rVals.R_rt;
           mips.memory[(val-0x00400000)/4] = rVals.R_rt;
          *changedMem = val;
       }
   }
   return val;
    //return val or else val will  be overwritten to 0;
}
